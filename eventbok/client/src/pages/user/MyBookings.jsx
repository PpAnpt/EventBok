...
const statusColor = {
  ...
  pending:   { bg: "#fef9c3", color: "#a16207", label: "Pending Payment" },
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 420, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.25)", padding: 24 }}>
        <h3 style={{ margin: "0 0 16px", fontSize: 16, fontWeight: 700 }}>Concert Ticket</h3>

        {seats.length === 0 ? (
          <p style={{ color: "#9ca3af", textAlign: "center", padding: 20 }}>No seat data found</p>
        ) : seats.map((seat, i) => (
          <div key={i} style={{ background: "#fff", borderRadius: 14, overflow: "hidden", border: "1px solid #e5e7eb", boxShadow: "0 2px 12px rgba(0,0,0,0.08)", marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", padding: "14px 18px", gap: 12, alignItems: "center" }}>
              <div style={{ flex: 1 }}>
                <Info label="Seat" value={seat.seat_no} big />
                <Info label="Zone" value={seat.zone_name} />
                <Info label="Date" value={booking.show_date} />
                <Info label="Session" value={booking.session_name} />
              </div>
              <div style={{ textAlign: "center" }}>
                ...
                <div style={{ fontSize: 9, color: "#9ca3af", marginTop: 4 }}>Scan to verify</div>
              </div>
            </div>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 24, fontWeight: 800, margin: "0 0 4px" }}>My Tickets</h1>
        <p style={{ color: "#6b7280", fontSize: 14, margin: 0 }}>Hello, {customer?.firstname} {customer?.lastname}</p>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: 60 }}>
          <p style={{ color: "#9ca3af", fontSize: 15, marginBottom: 16 }}>No bookings yet</p>
            ...
            <div key={b.booking_id} style={{ background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.07)", display: "flex" }}>
              ...
              <div style={{ flex: 1, padding: "16px 20px" }}>
                ...
                <div style={{ display: "flex", gap: 16, fontSize: 12, color: "#6b7280", marginBottom: 10, flexWrap: "wrap" }}>
                  ...
                  <span>Seats: {sortSeats(b.seats).map((s) => s.seat_no).join(", ") || "-"}</span>
